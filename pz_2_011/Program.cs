...
namespace pz_2_011
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            {
                fixed (char* x = a) // фиксируем указатель ура
                {
                    for (int i = 0; i < a.Length; i++) // ну просто цикл на проходку по длине слова введённого
                    {
                        if (!Char.IsLetter(x[i]) && !Char.IsDigit(x[i])) // спец символы не относятся к обычным символам юникода ну по идее вот поэтому такая логика
                        {
